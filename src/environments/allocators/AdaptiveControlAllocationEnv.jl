...
"""
struct AdaptiveControlAllocation <: AbstractEnv
    γ
    function AdaptiveControlAllocation(baseline; γ=1e-1)
        new(baseline, γ)
    end
end

function State(allocator::AdaptiveControlAllocation)
    return function (; Θ̂=zeros(6, 4))
        ComponentArray(Θ̂=Θ̂)
    end
end

function Dynamics!(allocator::AdaptiveControlAllocation)
